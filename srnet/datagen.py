...
class datagen_srnet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        i_t = imread(os.path.join(cfg.data_dir, cfg.i_t_dir, img_name))
        i_s = imread(os.path.join(cfg.data_dir, cfg.i_s_dir, img_name))
        t_sk = imread(os.path.join(cfg.data_dir, cfg.t_sk_dir, img_name), as_gray = True)
        t_t = imread(os.path.join(cfg.data_dir, cfg.t_t_dir, img_name))
        t_b = imread(os.path.join(cfg.data_dir, cfg.t_b_dir, img_name))
        t_f = imread(os.path.join(cfg.data_dir, cfg.t_f_dir, img_name))
        mask_t = imread(os.path.join(cfg.data_dir, cfg.mask_t_dir, img_name), as_gray = True)
...
class example_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        i_t = imread(os.path.join(self.datadir, img_name + 'i_t.png'))
        i_s = imread(os.path.join(self.datadir, img_name + 'i_s.png'))
        h, w = i_t.shape[:2]
