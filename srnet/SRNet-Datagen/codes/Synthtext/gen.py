...
class datagen():
    ...
    def __init__(self):
        ...
        src_font_dir = os.path.join(cur_file_path, data_cfg.src_font_dir)
        self.src_font_list = [os.path.join(src_font_dir, font_name) for font_name in os.listdir(src_font_dir)]
        # self.src_standard_font_path = os.path.join(cur_file_path, data_cfg.src_standard_font_path)
        
        trg_font_dir = os.path.join(cur_file_path, data_cfg.trg_font_dir)
        self.trg_font_list = [os.path.join(trg_font_dir, font_name) for font_name in os.listdir(trg_font_dir)]
        self.trg_standard_font_path = os.path.join(cur_file_path, data_cfg.trg_standard_font_path)
        ...
        src_text_filepath = os.path.join(cur_file_path, data_cfg.src_text_filepath)
        self.src_text_list = [text.strip() for text in open(src_text_filepath, 'r').readlines()]

        trg_text_filepath = os.path.join(cur_file_path, data_cfg.trg_text_filepath)
        self.trg_text_list = [text.strip() for text in open(trg_text_filepath, 'r').readlines()]
        ...
        self.bg_list = [
            os.path.join(cur_file_path, data_cfg.temp_bg_path+img_path.strip())
            for img_path in self.bg_list
        ]
    ...
    def gen_srnet_data_with_background(self):
        ...
        while True:
            ...
            src_font = np.random.choice(self.src_font_list)
            trg_font = np.random.choice(self.trg_font_list)
            
            text1, text2 = np.random.choice(self.src_text_list), np.random.choice(self.trg_text_list)
            ...
            # src_font = _init_font(src_font)
            # trg_font = _init_font(trg_font)

            def _init_fonts(*fonts):
                # choose font style
                size = np.random.randint(data_cfg.font_size[0], data_cfg.font_size[1] + 1)
                ...
                return [_init_font(font) for font in fonts]

            src_font, trg_font = _init_fonts(src_font, trg_font)
            ...
            surf1, bbs1 = render_text_mask.render_text(src_font, text1, param)
            param['curve_center'] = int(param['curve_center'] / len(text1) * len(text2))
            surf2, bbs2 = render_text_mask.render_text(trg_font, text2, param)
            ...
            i_t = render_standard_text.make_standard_text(self.trg_standard_font_path, text2, (surf_h, surf_w))
